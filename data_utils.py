...
path_tests = {'strategy_qa' : 'data/strategy_qa/strategyqa_train.json', 'commonsense_qa': 'data/commonsense_qa/dev_rand_split.json', 'open_book_qa': 'data/open_book_qa/test.json', 'worldtree': 'data/worldtree/test.json'}
...
def load_commonsense_qa(name='commonsense_qa', label_dict={'A': 0, 'B': 1, 'C':2, 'D':3, 'E':4}):
...
def load_cot_data(name='strategy_qa', num=100, ratio=0.25, label_dict={'True': 1, 'False': 0}):
    path = 'data/thoughtsource_100.json'
    path_test = path_tests[name]
    train_sentences, train_labels, test_sentences, test_labels = [], [], [], []
    with open(path, 'r') as file:
        ...
        if name == 'strategy_qa':
            data = json_data[name]['train']
        elif name in ['open_book_qa', 'worldtree']:
            data = json_data[name]['test']
        else:
            data = json_data[name]['validation']
        num = min(num, len(data))
        ...
        for i, json_data in enumerate(data):
            ...
            cot_in_generated_cot = []
            label = json_data['answer'][0]

            if name == 'strategy_qa':
                label = label_dict[json_data['answer'][0]]
                candidate = ' Choose the answer from True and False.'
            else:
                candidate = ''
                label = choices.index(label)
                for k in range(len(label_dict)):
                    candidate += '\n' + label_dict[k] + ' ' + choices[k] 
            
            if len(cot_in_generated_cot) == 0:
                random_item = ''
                for cot in json_data['cot']:
                    random_item += cot + ' '
                random_item = random_item[:-1]
            else:
                random_item = random.choice(cot_in_generated_cot)
...
def load_dataset(params):
    ...
    if params['dataset'] == 'sst2':
    ...
    elif 'mmlu' in params['dataset']:
        from load_mmlu import load_mmlu_cls, load_mmlu_cot
        if 'mmlu_cot' in params['dataset']:
            task = params['dataset'][9:]
            orig_train_sentences, orig_train_labels, orig_test_sentences, orig_test_labels, base_prompt = load_mmlu_cot(task)
        else:
            task = params['dataset'][5:]
            orig_train_sentences, orig_train_labels, orig_test_sentences, orig_test_labels, base_prompt = load_mmlu_cls(task)
        if params['subsample_test_set'] > len(orig_test_sentences):
            print(f'The maximum number of test instances is {len(orig_test_labels)}, which is smaller than the subsample_test_set {params["subsample_test_set"]}')
            raise AssertionError
        if params['num_shots'] > len(orig_train_sentences):
            print(f'The maximum number prompt/validation samples is {len(orig_train_sentences)}, which is smaller than num_shot {params["num_shots"]}')
            raise AssertionError
    ...
    elif params['dataset'] == 'worldtree_cot':
        params['prompt_prefix'] = ""
        params["q_prefix"] = "Question: "
        params["a_prefix"] = "Answer: "
        params['label_dict'] = {0:'A', 1:'B', 2:'C', 3:'D'}#{'A': 0, 'B': 1, 'C':2, 'D':3, 'E':4}
        ...
        params['task_format'] = 'classification'
        params['num_tokens_to_predict'] = 1
        orig_train_sentences, orig_train_labels, orig_test_sentences, orig_test_labels = load_cot_data(name='worldtree', label_dict=params['inv_label_dict'])
    ...
    elif params['dataset'] == 'open_book_qa_cot':
        params['prompt_prefix'] = ""
        params["q_prefix"] = "Question: "
        params["a_prefix"] = "Answer: "
        params['label_dict'] = {0:'A', 1:'B', 2:'C', 3:'D'}#{'A': 0, 'B': 1, 'C':2, 'D':3, 'E':4}
        ...
        params['task_format'] = 'classification'
        params['num_tokens_to_predict'] = 1
        orig_train_sentences, orig_train_labels, orig_test_sentences, orig_test_labels = load_cot_data(name='open_book_qa', label_dict=params['inv_label_dict'])
    elif params['dataset'] == 'commonsense_qa':
        params['prompt_prefix'] = ""
        params["q_prefix"] = "Question: "
        params["a_prefix"] = "Answer: "
        params['label_dict'] = {0:'A', 1:'B', 2:'C', 3:'D', 4:'E'}#{'A': 0, 'B': 1, 'C':2, 'D':3, 'E':4}
        params['inv_label_dict'] = {0:'A', 1:'B', 2:'C', 3:'D', 4:'E'}
        params['task_format'] = 'classification'
        params['num_tokens_to_predict'] = 1
        orig_train_sentences, orig_train_labels, orig_test_sentences, orig_test_labels = load_commonsense_qa(name='commonsense_qa', label_dict=params['inv_label_dict'])
    elif params['dataset'] == 'commonsense_qa_cot':
        params['prompt_prefix'] = ""
        params["q_prefix"] = "Question: "
        params["a_prefix"] = "Answer: "
        params['label_dict'] = {0:'A', 1:'B', 2:'C', 3:'D', 4:'E'}#{'A': 0, 'B': 1, 'C':2, 'D':3, 'E':4}
        ...
        params['task_format'] = 'classification'
        params['num_tokens_to_predict'] = 1
        orig_train_sentences, orig_train_labels, orig_test_sentences, orig_test_labels = load_cot_data(name='commonsense_qa', label_dict=params['inv_label_dict'])

    elif params['dataset'] == 'strategy_qa_cot':
        params['prompt_prefix'] = ""
        params["q_prefix"] = "Question: "
        params["a_prefix"] = "Answer: "
        params['label_dict'] = {1: ['True'], 0: ['False']}
        ...
        params['task_format'] = 'classification'
        params['num_tokens_to_predict'] = 1
        orig_train_sentences, orig_train_labels, orig_test_sentences, orig_test_labels = load_cot_data(name='strategy_qa', label_dict=params['inv_label_dict'])
